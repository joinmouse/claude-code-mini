...
本项目用 **3753 行 TypeScript** 复现了 Claude Code 的核心架构——Agent Loop、13 个工具（含并行执行 + 流式早期启动）、4 层上下文压缩、语义记忆召回、技能系统、多 Agent、MCP 集成。每一步都对照真实源码讲解"它怎么做的 → 我们怎么简化的"。
...
按教程 Phase 划分，代码组织与文档章节严格对应（**共 3753 行**）：

```
src/
├── core/                    # Phase 1 · 构建可用的 Coding Agent（Ch1-7, 2611 行）
│   ├── agent.ts             # Ch1  Agent 循环：流式、并行执行、预算    (795 行)
│   ├── tools.ts             # Ch2  13 工具 + mtime 防护 + 延迟加载     (827 行)
│   ├── prompt.ts            # Ch3  System Prompt：@include + 模板注入 (232 行)
│   ├── cli.ts               # Ch4  CLI 入口：参数解析、REPL           (316 行)
