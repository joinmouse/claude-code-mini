...
| mini-claude (TypeScript) | Claude Code 源码 | 说明 |
|------------|-------------------|------|
...
没实现的原因：需要管理 LSP 服务器进程、实现客户端协议（初始化握手、能力协商、增量同步），1000+ 行且依赖对 LSP 协议的深入理解。通过 shell 命令（`tsc --noEmit`）获得错误反馈，对教程场景已经足够。
...
核心转变是**从硬编码到插件化**。当前 switch/case 在 10 个工具时没问题，但超过 20 个就需要引入 Tool 接口，让每个工具成为独立模块。
...
~3400 行代码（TypeScript），11 个文件，覆盖了一个 coding agent 的核心组件和进阶能力：

**Phase 1 — 核心组件：** Agent Loop、工具系统（13 工具 + mtime 防护 + 延迟加载 + 并行执行）、System Prompt（Markdown 模板 + @include + 环境注入）、CLI / 会话（REPL + JSON 持久化）、流式输出（Anthropic streaming + streaming 工具执行）、权限安全（5 模式 + 声明式规则 + 正则 + 确认）、上下文管理（4 层压缩 + 大结果持久化）
